...
Descripcion:
  Firmware de nodo sensor variante B, capta el pulso INT_SINC y gestiona
  adquisicion ADXL355 + mensajes RS485 para el segundo nodo de la red.
...
#define IDNODO 2                                                                // ID RS485 del nodo: 2 = Nodo B
...
// Pines y perifericos
struct sdflags sdflags;                                                         // Estructura de estado de la SD
sbit TEST at LATA2_bit;                                                         // LED TEST1
sbit TEST_Direction at TRISA2_bit;

sbit CsADXL at LATA3_bit;                                                       // CS del ADXL355 (SPI2)
sbit CsADXL_Direction at TRISA3_bit;

sbit sd_CS_lat at LATB0_bit;                                                    // CS de la MicroSD
sbit sd_CS_tris at TRISB0_bit;
sbit sd_detect_port at LATA4_bit;                                               // Deteccion de tarjeta SD
sbit sd_detect_tris at TRISA4_bit;

sbit MSRS485 at LATB12_bit;                                                     // DE/RE del MAX485 de datos
sbit MSRS485_Direction at TRISB12_bit;

// El MAX485 U5 esta en modo recepcion permanente; su salida INT_SINC va
// directo a la interrupcion externa INT1.

// Estado general del nodo
unsigned short inicioSistema;

// Tiempo del sistema
unsigned short tiempo[6] = {0, 0, 0, 0, 0, 0};                                  // Registro de fecha/hora recibido por RS485
...
unsigned short contTimer1;                                                      // Desbordamientos de TMR1 entre pulsos de sync
...
unsigned short contTMR2;                                                        // Timeout de recuperacion de UART1
